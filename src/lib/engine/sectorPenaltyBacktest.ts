...
import { portfolioMetric } from "./strategyValidationPortfolio";
...
function createAgg(code: string, name: string): Agg {
  return {
    sectorCode: code, sectorName: name, memberCount: 0, r20: [], r60: [], r120: [],
    turnover5: 0, turnover20: 0, turnover5Count: 0, turnover20Count: 0,
    aboveMa20True: 0, aboveMa20Valid: 0, aboveMa60True: 0, aboveMa60Valid: 0,
    aboveMa120True: 0, aboveMa120Valid: 0, maAlignedTrue: 0, maAlignedValid: 0,
    nearHighTrue: 0, nearHighValid: 0, advancingTrue: 0, advancingValid: 0,
  };
}
...
function buildSectorPriceLeadershipMap(dataset: MarketDataset) {
  ...
  const prepared = dataset.instruments
    .filter((inst) => inst.instrumentType === "STOCK" && inst.isActive && inst.sectorCode !== "MARKET_IDX" && inst.sectorCode !== "ETC")
  ...
  for (let marketIndex = 120; marketIndex < kospi.bars.length; marketIndex++) {
    ...
    for (const item of prepared) {
      ...
      if (finite(tv5)) { agg.turnover5 += tv5; agg.turnover5Count++; }
      if (finite(tv20)) { agg.turnover20 += tv20; agg.turnover20Count++; }
      ...
      addBool(agg, "nearHigh", Number.isFinite(item.rollingHigh250[i]) ? bar.close >= item.rollingHigh250[i]! * 0.95 : null);
      addBool(agg, "advancing", bar.close > prev.close);
      aggs.set(code, agg);
    }

    const raw: RawSectorRow[] = [...aggs.values()].map((agg) => {
      const eq20 = median(agg.r20), eq60 = median(agg.r60), eq120 = median(agg.r120);
      return {
        ...
        relativeTurnover: agg.turnover20Count > 0 && agg.turnover5Count > 0 && agg.turnover20 > 0 ? agg.turnover5 / agg.turnover20 : null,
...
function crossedUp(prev: number | null | undefined, cur: number | null | undefined, threshold: number) {
  const p = percent(prev), c = percent(cur);
...
function scoreRise(s: Series, i: number, lag: number) {
  const c = percent(s.scores[i]), p = percent(s.scores[i - lag]);
  return c !== null && p !== null ? c - p : null;
}
function excursion(s: Series, entryIndex: number, exitIndex: number, entryPrice: number, exitPrice: number, timing: "OPEN" | "CLOSE" | "STOP") {
...
function benchmarkReturn(bench: Map<string, DailyPrice> | undefined, entryDate: string, exitDate: string, timing: "OPEN" | "CLOSE" | "STOP") {
  const entry = bench?.get(entryDate), exit = bench?.get(exitDate);
...
function makeTrade(s: Series, scenario: SectorPenaltyScenario, signalIndex: number, entryIndex: number, exitIndex: number, entryPrice: number, exitPrice: number, reason: V6ExitReason, timing: "OPEN" | "CLOSE" | "STOP", costBps: number, bench: Map<string, DailyPrice> | undefined): SectorPenaltyTrade {
  const entryDate = s.bars[entryIndex]!.tradeDate, exitDate = s.bars[exitIndex]!.tradeDate;
  ...
  return {
    symbol: s.symbol, ...(s.name !== undefined ? { name: s.name } : {}), market: s.market,
    signalDate: s.bars[signalIndex]!.tradeDate, entryDate, exitDate, signalIndex, entryIndex, exitIndex,
    entryPrice, exitPrice, ret, holdingDays: exitIndex - entryIndex + 1, reason, exitTiming: timing,
    signalScore: percent(s.scores[signalIndex]) ?? 0, scoreChange1d: scoreRise(s, signalIndex, 1),
    scoreRise5d: scoreRise(s, signalIndex, 5), scoreRise10d: scoreRise(s, signalIndex, 10),
    signalRegime: s.regimes[signalIndex] ?? "UNKNOWN", mae: ex.mae, mfe: ex.mfe,
    modelId: scenario.modelId, priceLeadershipOverheatThreshold: scenario.priceLeadershipOverheatThreshold,
    baseScore9p5: s.baseScores[signalIndex] ?? null, adjustedScore10: s.scores[signalIndex]!,
    sectorCode: s.sectorCode, sectorName: s.sectorName,
    ...
    benchmarkReturn: br, excessReturn: br === null ? null : ret - br,
  };
}
...
function simulateTrade(s: Series, signalIndex: number, scenario: SectorPenaltyScenario, costBps: number, bench: Map<string, DailyPrice> | undefined) {
  const entryIndex = signalIndex + 1, plannedExit = signalIndex + scenario.maxHoldingDays;
  ...
  for (let j = entryIndex; j <= Math.min(plannedExit, s.bars.length - 1); j++) {
    ...
    if (j === plannedExit) return makeTrade(s, scenario, signalIndex, entryIndex, j, entry.open, bar.close, "TIME", "CLOSE", costBps, bench);
  }
  return null;
}
function simulateScenario(s: Series, scenario: SectorPenaltyScenario, costBps: number, benchmarks: Record<Market, Map<string, DailyPrice> | undefined>) {
  ...
  for (let i = 1; i + 1 < s.bars.length; i++) {
    ...
    if (trade) { trades.push(trade); nextSignalIndex = trade.exitIndex + 1; }
  }
  return trades;
}
function scenariosFor(modelId: string, modelLabel: string, threshold: number | null, entries: number[], ups: number[], downs: number[], holds: number[]) {
  return entries.flatMap((entry) => holds.flatMap((hold) => ups.flatMap((up) => downs.map((down): SectorPenaltyScenario => ({
    id: `${modelId}-e${entry}-h${hold}-u${up}-d${down}`,
    label: `${modelLabel} · ${entry} Onset · 최대 ${hold}D · ↑${up} / ↓${down}`,
    modelId, modelLabel, priceLeadershipOverheatThreshold: threshold,
    entryThreshold: entry, upsideExitThreshold: up, downsideExitThreshold: down, maxHoldingDays: hold,
  }))));
}
  ...
  return [...dataset.instruments]
    ...
    .map((inst): Series | null => {
      ...
      for (let i = 120; i < bars.length; i++) {
        ...
        scores[i] = score; baseScores[i] = score;
        nearHighs[i] = snap.distanceFrom52wHigh === null ? null : snap.distanceFrom52wHigh >= -10;
        extensions[i] = snap.extensionFromMa20;
      }
      return {
        symbol: inst.symbol, name: inst.name, market: inst.market === "KOSDAQ" ? "KOSDAQ" : "KOSPI",
        sectorCode: inst.sectorCode, sectorName: inst.sectorName, bars, scores, baseScores, nearHighs, extensions,
...
function applyModel(source: Series[], sectorPriceByDate: Map<string, number>, threshold: number | null) {
  ...
  let scoredDays = 0, penaltyAppliedDays = 0;
  const bases: number[] = [], adjusted: number[] = [], sectorScores: number[] = [];
  const series = source.map((s): Series => {
    ...
    for (let i = 0; i < s.scores.length; i++) {
      ...
      scoredDays++; if (overheated) penaltyAppliedDays++;
      bases.push(base); adjusted.push(scores[i]!); if (pl !== null) sectorScores.push(pl);
    }
    return { ...s, scores, sectorPriceLeadership, sectorOverheated };
  });
  return { series, summary: { modelId, modelLabel, priceLeadershipOverheatThreshold: threshold, scoredDays, penaltyAppliedDays,
    penaltyAppliedRate: scoredDays ? (penaltyAppliedDays / scoredDays) * 100 : null, avgBaseScore9p5: mean(bases),
    avgAdjustedScore10: mean(adjusted), avgSectorPriceLeadership: mean(sectorScores) } };
}
function filterTrades(trades: SectorPenaltyTrade[], split: "ALL" | "OOS", market: "ALL" | Market, oosStart: string | null) {
  return trades.filter((t) => (split === "ALL" || (oosStart !== null && t.signalDate >= oosStart)) && (market === "ALL" || t.market === market));
}
function summarize(scenario: SectorPenaltyScenario, trades: SectorPenaltyTrade[], split: "ALL" | "OOS", market: "ALL" | Market, portfolio: ReturnType<typeof portfolioMetric>): SectorPenaltyMetricRow {
  const returns = trades.map((t) => t.ret), wins = returns.filter((r) => r > 0), losses = returns.filter((r) => r < 0);
  const excess = trades.map((t) => t.excessReturn).filter(finite), excessWins = excess.filter((r) => r > 0);
  const avgWin = mean(wins), avgLoss = mean(losses);
  const lossSum = losses.reduce((a, b) => a + b, 0), winSum = wins.reduce((a, b) => a + b, 0);
  return {
    scenario: scenario.id, label: scenario.label, modelId: scenario.modelId, modelLabel: scenario.modelLabel,
    split, market, priceLeadershipOverheatThreshold: scenario.priceLeadershipOverheatThreshold,
    entryThreshold: scenario.entryThreshold, entryScore10: scenario.entryThreshold / 10,
    upsideExitThreshold: scenario.upsideExitThreshold, upsideExitScore10: scenario.upsideExitThreshold / 10,
    downsideExitThreshold: scenario.downsideExitThreshold, downsideExitScore10: scenario.downsideExitThreshold / 10,
    maxHoldingDays: scenario.maxHoldingDays, trades: trades.length, avgReturn: mean(returns), medianReturn: median(returns),
    winRate: trades.length ? (wins.length / trades.length) * 100 : null, avgExcessReturn: mean(excess), medianExcessReturn: median(excess),
    ...
    averageHoldingDays: mean(trades.map((t) => t.holdingDays)), timeExitRate: pct(trades, (t) => t.reason === "TIME"),
    upsideExitRate: pct(trades, (t) => t.reason === "UPSIDE_SCORE"), downsideExitRate: pct(trades, (t) => t.reason === "DOWNSIDE_SCORE"),
    avgSignalScore: mean(trades.map((t) => t.signalScore)), avgBaseScore9p5: mean(trades.map((t) => t.baseScore9p5).filter(finite)),
    avgSectorPriceLeadership: mean(trades.map((t) => t.sectorPriceLeadership).filter(finite)), penaltyAppliedRate: pct(trades, (t) => t.sectorPenaltyApplied),
    tradesWithSectorScoreRate: pct(trades, (t) => t.sectorPriceLeadership !== null), worstReturn: returns.length ? Math.min(...returns) : null,
    avgMae: mean(trades.map((t) => t.mae).filter(finite)), avgMfe: mean(trades.map((t) => t.mfe).filter(finite)),
    portfolioTotalReturn: portfolio.totalReturn, portfolioCagr: portfolio.cagr, portfolioMdd: portfolio.mdd, portfolioSharpe: portfolio.sharpe,
    activeDayRate: portfolio.activeDayRate, avgActivePositions: portfolio.avgActivePositions, peakActivePositions: portfolio.peakActivePositions,
  };
}
function best(rows: SectorPenaltyMetricRow[], split: "ALL" | "OOS", metric: "avgExcessReturn" | "portfolioSharpe") {
  return rows.filter((r) => r.split === split && r.market === "ALL" && r.trades >= 20 && r[metric] !== null).sort((a, b) => (b[metric] ?? -Infinity) - (a[metric] ?? -Infinity))[0] ?? null;
}

export function runSectorPenaltyBacktest(dataset: MarketDataset, options: SectorPenaltyBacktestOptions = {}): SectorPenaltyBacktestResult | null {
  ...
  const benchmarkMaps = {
    KOSPI: benchmarks.KOSPI ? new Map(benchmarks.KOSPI.map((b) => [b.tradeDate, b])) : undefined,
    KOSDAQ: benchmarks.KOSDAQ ? new Map(benchmarks.KOSDAQ.map((b) => [b.tradeDate, b])) : undefined,
  };
  const scoreModels: SectorPenaltyScoreModelSummary[] = [];
  const rows: SectorPenaltyMetricRow[] = [];
  const thresholds: Array<number | null> = [...(includeNoPenaltyBaseline ? [null] : []), ...overheatThresholds];
    ...
    for (const scenario of scenarios) {
      ...
      for (const split of ["ALL", "OOS"] as const) {
        for (const market of ["ALL", "KOSPI", "KOSDAQ"] as const) {
          ...
          const startDate = split === "OOS" ? oosStart : group.map((t) => t.entryDate).sort()[0] ?? null;
          const portfolio = portfolioMetric(group, series, allDates, startDate, cost);
          rows.push(summarize(scenario, group, split, market, portfolio));
        }
      }
    }
  }
  const defaultRows = rows.filter((r) => r.priceLeadershipOverheatThreshold === 80 && r.entryThreshold === 75 && r.upsideExitThreshold === 90 && r.downsideExitThreshold === 30 && r.maxHoldingDays === 30);
  return {
    version: SECTOR_PENALTY_BACKTEST_VERSION,
    from: allDates[0] ?? "", to: allDates.at(-1) ?? "", symbolCount: source.length,
    sectorCount: new Set(source.map((s) => s.sectorCode)).size,
    scoreMax: 10, sectorSlotPoints: 0.5, sectorPenaltyPoints: 0.5,
    defaultScenario: { entryThreshold: 75, entryScore10: 7.5, upsideExitThreshold: 90, upsideExitScore10: 9, downsideExitThreshold: 30, downsideExitScore10: 3, maxHoldingDays: 30, priceLeadershipOverheatThreshold: 80 },
    compared: { entryThresholds: entries, upsideExitThresholds: ups, downsideExitThresholds: downs, maxHoldingDays: holds, priceLeadershipOverheatThresholds: overheatThresholds, includeNoPenaltyBaseline },
    scoreModels, rows, defaultRows,
    bestRows: { allByAvgExcess: best(rows, "ALL", "avgExcessReturn"), oosByAvgExcess: best(rows, "OOS", "avgExcessReturn"), allBySharpe: best(rows, "ALL", "portfolioSharpe"), oosBySharpe: best(rows, "OOS", "portfolioSharpe") },
    notes: [
