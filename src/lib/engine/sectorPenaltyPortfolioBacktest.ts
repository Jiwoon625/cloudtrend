...
export interface SectorPenaltyPortfolioBacktestOptions {
  ...
  signalContext?: PortfolioSignalContext;
}
...
export interface SectorPenaltyPortfolioBacktestResult {
  ...
  optimizationStats: {
    ...
    thresholdSeriesCopies: 0;
    requestedMetricRows: number;
    uniquePortfolioSimulations: number;
    reusedPortfolioSimulations: number;
  };
...
function simulationKey(s: PortfolioStrategyDefinition, split: PortfolioSplit, market: PortfolioMarket, cost: number, maxPositions: number, mode: PositionWeightMode) { return [s.id, split, market, `c${cost}`, `p${maxPositions}`, `w${targetWeight(mode, maxPositions).toFixed(8)}`].join("|"); }
...
function lookupSeries(series: PortfolioSeries[]) { return new Map(series.map((s) => [key(s.market, s.symbol), s])); }
...
function simulatePortfolio(candidates: PortfolioCandidateTrade[], series: PortfolioSeries[], dates: string[], strategy: PortfolioStrategyDefinition, initialCapital: number, roundTripCostBps: number, maxPositions: number, weightMode: PositionWeightMode): SimulationResult {
  ...
  const closePosition = (positionKey: string, position: Position, price: number) => { const exitGross = position.shares * price, exitFee = exitGross * halfCost; cash += exitGross - exitFee; totalFees += exitFee; sellNotional += exitGross; const costBasis = position.entryNotional + position.entryFee; const netReturn = costBasis > 0 ? ((exitGross - exitFee) / costBasis - 1) * 100 : 0; accepted.push({ ...position.candidate, entryNotional: position.entryNotional, entryFee: position.entryFee, exitGross, exitFee, netReturn }); positions.delete(positionKey); };
  for (const date of dates) {
    ...
    for (const candidate of byEntry.get(date) ?? []) {
      const positionKey = key(candidate.market, candidate.symbol); if (positions.has(positionKey)) { skippedAlreadyHeld++; continue; } if (positions.size >= maxPositions) { skippedForCapacity++; continue; }
      const targetNotional = openEquity * weight, maxAffordable = cash / (1 + halfCost), notionalBudget = Math.min(targetNotional, maxAffordable), shares = Math.floor(notionalBudget / candidate.entryPrice); if (shares < 1) { skippedForCash++; continue; }
    ...
    const marked = [...positions.values()].reduce((sum, p) => sum + p.shares * p.lastMark, 0), equity = cash + marked; peak = Math.max(peak, equity); const dailyReturn = previousEquity > 0 ? (equity / previousEquity - 1) * 100 : 0, drawdown = peak > 0 ? (equity / peak - 1) * 100 : 0; points.push({ date, equity, dailyReturn, cash, cashWeight: equity > 0 ? cash / equity * 100 : 100, activePositions: positions.size, drawdown }); previousEquity = equity;
  }
...
function periodReturns(points: PortfolioEquityPoint[], keyFn: (date: string) => string) { const groups = new Map<string, number[]>(); for (const point of points) { const k = keyFn(point.date), list = groups.get(k) ?? []; list.push(point.dailyReturn / 100); groups.set(k, list); } return [...groups].map(([period, rs]) => ({ period, return: compound(rs)! })); }

function benchmarkRegimeMap(dataset: MarketDataset, market: PortfolioMarket) {
  const maps = new Map<string, number[]>(), codes = market === "ALL" ? ["KOSPI", "KOSDAQ"] : [market];
...
function practicalRows(rows: SectorPenaltyPortfolioMetricRow[]) { return rows.filter((r) => r.split === "ALL" && r.market === "ALL" && r.roundTripCostBps === 15 && r.maxPositions === 10 && r.weightMode === "MAX_10" && r.trades >= 20).sort((a, b) => { const as = (a.cagr ?? -100) + (a.sharpe ?? -10) * 10 + (a.profitFactor ?? 0) * 5 + (a.mdd ?? -100) * 0.2, bs = (b.cagr ?? -100) + (b.sharpe ?? -10) * 10 + (b.profitFactor ?? 0) * 5 + (b.mdd ?? -100) * 0.2; return bs - as; }).slice(0, 10); }
...
export function runSectorPenaltyPortfolioBacktest(dataset: MarketDataset, options: SectorPenaltyPortfolioBacktestOptions = {}): SectorPenaltyPortfolioBacktestResult | null {
  ...
  const context = options.signalContext ?? buildPortfolioSignalContext(dataset, limit); if (!context.symbolCount) return null; const series = context.series, allDates = calendarFor(dataset, "ALL"), from = allDates[0] ?? "", to = allDates.at(-1) ?? "", oosStart = allDates[Math.max(0, Math.floor(allDates.length * 0.8))] ?? null;
  const rows: SectorPenaltyPortfolioMetricRow[] = [], yearlyReturns: PeriodReturnRow[] = [], monthlyReturns: PeriodReturnRow[] = [], regimeReturns: RegimeReturnRow[] = [], candidateCache = new Map<string, PortfolioCandidateTrade[]>(), simulationCache = new Map<string, SimulationResult>();
  for (const strategy of PORTFOLIO_STRATEGIES) {
    const candidates = buildPortfolioCandidates(series, strategy); candidateCache.set(strategy.id, candidates);
    for (const market of ["ALL", "KOSPI", "KOSDAQ"] as const) {
      ...
      for (const split of ["ALL", "OOS"] as const) {
        ...
        for (const cost of costs) for (const maxPositions of positionLimits) for (const mode of weightModes) {
          const cacheKey = simulationKey(strategy, split, market, cost, maxPositions, mode); let simulation = simulationCache.get(cacheKey); if (!simulation) { simulation = simulatePortfolio(splitCandidates, marketSeries, dates, strategy, initialCapital, cost, maxPositions, mode); simulationCache.set(cacheKey, simulation); }
          const row = metricRow(strategy, split, market, cost, maxPositions, mode, initialCapital, simulation); rows.push(row);
  ...
  const byCagr = selectBest(rows, "cagr"), bySharpe = selectBest(rows, "sharpe"), practicalAllMarket = practicalRows(rows), requestedCurveRows = [...byCagr, ...bySharpe, ...practicalAllMarket, ...rows.filter((r) => r.scenario === "baseline-e75-u90-d30-h30" && r.split === "ALL" && r.market === "ALL" && r.roundTripCostBps === 15 && r.maxPositions === 10 && r.weightMode === "MAX_10")], curveKeys = new Set(requestedCurveRows.map((r) => r.rowKey));
  const equityCurve: CurveBundle[] = [], drawdownSeries: Array<{ rowKey: string; points: Array<{ date: string; drawdown: number }> }> = [];
  for (const curveKey of curveKeys) { const row = rows.find((r) => r.rowKey === curveKey); if (!row) continue; const strategy = PORTFOLIO_STRATEGIES.find((s) => s.id === row.scenario); if (!strategy) continue; const simulation = simulationCache.get(simulationKey(strategy, row.split, row.market, row.roundTripCostBps, row.maxPositions, row.weightMode)); if (!simulation) continue; equityCurve.push({ rowKey: row.rowKey, scenario: row.scenario, market: row.market, split: row.split, roundTripCostBps: row.roundTripCostBps, maxPositions: row.maxPositions, weightMode: row.weightMode, points: simulation.points }); drawdownSeries.push({ rowKey: row.rowKey, points: simulation.points.map((p) => ({ date: p.date, drawdown: p.drawdown })) }); }
  return {
    ...
    optimizationStats: { featureCacheUsed: context.featureCacheUsed, thresholdSeriesCopies: 0, requestedMetricRows: rows.length, uniquePortfolioSimulations: simulationCache.size, reusedPortfolioSimulations: Math.max(0, rows.length - simulationCache.size) },
    rows, bestRows: { byCagr, bySharpe, practicalAllMarket }, yearlyReturns, monthlyReturns, regimeReturns, equityCurve, drawdownSeries,
    notes: ["직전 V8 Fast와 같은 9.5점 기술점수 + 섹터 슬롯 0.5점 - 섹터 과열 페널티 0.5점 산식을 사용한다.", "PL 임계값별 전체 점수 시계열 복제를 제거하고 공통 baseScore + sectorPL feature에서 전략별 adjustedScore를 계산한다.", "동일한 실질 목표비중을 만드는 weightMode 조합은 하나의 포트폴리오 시뮬레이션을 공유하되 기존 결과 rowKey와 출력 행은 유지한다.", "후보는 모든 새 onset을 생성한 뒤 실제 보유 중인 종목만 중복 진입을 차단한다. 용량 부족으로 놓친 onset은 지연 진입하지 않고 이후 새 onset이 생겨야 다시 후보가 된다.", "점수 청산은 교차 확인 다음 거래일 시가, 최대보유 청산은 계획 만기일 종가다.", "진입 시 고정 슬롯 비중으로 정수 주식을 매수하고 자동 리밸런싱하지 않으며 미투자 현금을 허용한다.", "MAX_20은 테스트한 최대보유 5/10/20에서 동일 슬롯 비중과 같고, MAX_10은 최대 5종목 설정에서 추가 현금 버퍼를 만든다.", "OOS는 전체 거래일 마지막 20% 시작일부터 새 신호만 허용하고 현금 100%로 재시작한다.", "시장국면 성과는 독립 구간 백테스트가 아니라 해당 국면 일별 포트폴리오 수익률의 조건부 복리다."]
  };
}
