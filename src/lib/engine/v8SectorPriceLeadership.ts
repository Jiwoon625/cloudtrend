...
function combine(parts: Array<{ weight: number; ratio: number | null }>) {
  ...
  return (
    (available.reduce((sum, part) => sum + part.weight * (part.ratio ?? 0), 0) / weight) *
    100
  );
}
...
/**
 * V8 Final technical score's 0.5-point Sector Price Leadership slot.
 * This reproduces the price-leadership definition used by the strict 3-FOS research,
 * but computes only the requested snapshot offset so live screening stays lightweight.
 */
export function computeV8SectorPriceLeadership(
  ...
  for (const instrument of dataset.instruments) {
    ...
    const index = bars.length - 1 - offset;
    ...
    addFlag(
      ...
      "aboveMa20Yes",
      "aboveMa20Valid",
      ma20 === null ? null : close > ma20,
    );
    addFlag(
      aggregate,
      "aboveMa60Yes",
      "aboveMa60Valid",
      ma60 === null ? null : close > ma60,
    );
    addFlag(
      aggregate,
      "aboveMa120Yes",
      "aboveMa120Valid",
      ma120 === null ? null : close > ma120,
    );
    addFlag(
      aggregate,
      "maAlignedYes",
    ...
    addFlag(
      ...
      "nearHighYes",
      "nearHighValid",
      high52 === null ? null : close >= high52 * 0.95,
    );

    const turnover5 = averageTradingValue(tradingValues, index, 5);
    const turnover20 = averageTradingValue(tradingValues, index, 20);
    if (finite(turnover5)) {
      aggregate.turnover5 += turnover5;
      aggregate.turnover5Valid += 1;
    }
    if (finite(turnover20)) {
      aggregate.turnover20 += turnover20;
      aggregate.turnover20Valid += 1;
    }
  ...
  const raw = [...aggregates.entries()].map(([sectorCode, aggregate]) => {
    ...
    return {
      ...
      relativeTurnover:
        aggregate.turnover5Valid > 0 &&
        aggregate.turnover20Valid > 0 &&
        aggregate.turnover20 > 0
  ...
  for (const row of raw) {
    const trend = [row.aboveMa20, row.aboveMa60, row.aboveMa120]
      .filter(finite)
      .map((value) => value / 100);
    const breadth = [row.advancing, row.aboveMa20, row.maAligned]
      .filter(finite)
      .map((value) => value / 100);
    const score = combine([
      ...
      { weight: 10, ratio: row.nearHigh === null ? null : row.nearHigh / 100 },
      {
        weight: 5,
        ratio:
          row.relativeTurnover === null ? null : clamp01((row.relativeTurnover - 0.7) / 0.8),
      },
    ]);
