...
export type SectorScoreKind = "PRICE" | "FLOW" | "ROTATION";
...
const SCORE_KINDS: SectorScoreKind[] = ["PRICE", "FLOW", "ROTATION"];
...
function buildRawSectorDay(
  ...
  const market20 = periodReturn(kospiBars, marketIndex, 20);
  const market60 = periodReturn(kospiBars, marketIndex, 60);
  const market120 = periodReturn(kospiBars, marketIndex, 120);
  const complete = sectorDefs
    .map((sector) => aggregates.get(sector.code))
    .filter((item): item is SectorAggregate => item !== undefined && item.memberCount > 0);
  const total5 = complete.reduce((sum, item) => sum + (item.turnover5Count ? item.turnover5 : 0), 0);
  const total20 = complete.reduce((sum, item) => sum + (item.turnover20Count ? item.turnover20 : 0), 0);
  ...
  return complete.map((aggregate) => {
    ...
    return {
      ...
      rs20: finite(eq20) && finite(market20) ? (eq20 - market20) * 100 : null,
      rs60: finite(eq60) && finite(market60) ? (eq60 - market60) * 100 : null,
      rs120: finite(eq120) && finite(market120) ? (eq120 - market120) * 100 : null,
      ...
      foreign5: aggregate.foreign5Valid ? aggregate.foreign5Sum : null,
      institution5: aggregate.institution5Valid ? aggregate.institution5Sum : null,
      turnover20Total:
...
function rankRows(
  level: Array<LevelScore & { rotationScore: number }>,
  kind: SectorScoreKind,
): RankedSector[] {
  const scoreOf = (row: LevelScore & { rotationScore: number }) =>
    kind === "PRICE" ? row.price : kind === "FLOW" ? row.flow : row.rotationScore;
  return level
    .map((row) => ({ row, score: scoreOf(row) }))
    .filter((item): item is { row: LevelScore & { rotationScore: number }; score: number } => finite(item.score))
    .sort((a, b) => b.score - a.score || a.row.sectorCode.localeCompare(b.row.sectorCode))
    .map(({ row, score }, index) => ({
      sectorCode: row.sectorCode,
      sectorName: row.sectorName,
      score,
...
function buildRankHistory(
  ...
  for (let marketIndex = MIN_LEVEL_INDEX; marketIndex < kospi.bars.length; marketIndex++) {
    ...
    const previousByCode = new Map(previous.map((row) => [row.sectorCode, row]));
    const drafts = level.map((current) => {
      const prev = previousByCode.get(current.sectorCode);
      return {
        ...current,
        momentumParts: [
    ...
    const momentumSeries = [0, 1, 2, 3].map((part) =>
      drafts.map((draft) => draft.momentumParts[part]).filter(finite).sort((a, b) => a - b),
    );
    const scored = drafts.map((draft) => {
      const momentumRatios = draft.momentumParts
        .map((value, part) => percentileRatio(momentumSeries[part]!, value))
        .filter(finite);
      const rotationMomentum = momentumRatios.length ? mean(momentumRatios)! * 100 : null;
      const weighted = [
      ...
      const rotationScore = availableWeight
        ...
        : 0;
      return { ...draft, rotationScore };
    });
    const byKind = Object.fromEntries(
      SCORE_KINDS.map((kind) => [kind, rankRows(scored, kind)]),
    ) as Record<SectorScoreKind, RankedSector[]>;
    if (SCORE_KINDS.some((kind) => byKind[kind].length !== sectorDefs.length)) continue;
    output.push({ date, marketIndex, byKind });
  }
  return output;
}
...
function strictTopEpisodes(
  ...
  for (const sector of sectorDefs) {
    ...
    history.forEach((day, position) => {
      ...
      const inTop = row.rank <= TOP_MAX_RANK;
      if (!current && inTop) {
        current = {
...
function survival(episodes: Episode[], horizon: number) {
...
function topSetRetention(
  ...
  for (const day of history) {
    ...
    if (!future) continue;
    const currentTop = new Set(day.byKind[kind].slice(0, TOP_MAX_RANK).map((row) => row.sectorCode));
    const futureTop = new Set(future.byKind[kind].slice(0, TOP_MAX_RANK).map((row) => row.sectorCode));
    let overlap = 0;
    for (const code of currentTop) if (futureTop.has(code)) overlap++;
    retained.push(overlap);
  }
  const avgRetained = mean(retained);
  return {
    horizon,
    observations: retained.length,
    avgRetained,
    avgRetentionRate: avgRetained === null ? null : (avgRetained / TOP_MAX_RANK) * 100,
    avgNewEntrants: avgRetained === null ? null : TOP_MAX_RANK - avgRetained,
  };
}
...
function boundaryGap(history: DailyRanking[], kind: SectorScoreKind): BoundaryGapStat {
  const gaps = history
    .map((day) => {
      ...
      return rows.length >= 5 ? rows[3]!.score - rows[4]!.score : null;
    })
    .filter(finite);
  return {
...
function flowConfirmation(history: DailyRanking[]): FlowConfirmationStat {
  ...
  let combinedAndShare = 0;
  let bothAndShare = 0;
  let negativeCombined = 0;
  let newTop4Entries = 0;
  let entryCombinedAndShare = 0;
  let entryBothAndShare = 0;
  const byIndex = new Map(history.map((day) => [day.marketIndex, day]));

  for (const day of history) {
    const top = day.byKind.FLOW.slice(0, TOP_MAX_RANK);
    for (const row of top) {
      const combined =
        finite(row.foreign5) && finite(row.institution5) ? row.foreign5 + row.institution5 : null;
      const combinedIsPositive = finite(combined) && combined > 0;
      const combinedIsNegative = finite(combined) && combined < 0;
      const bothPositive =
        finite(row.foreign5) && row.foreign5 > 0 && finite(row.institution5) && row.institution5 > 0;
      const sharePositive = row.turnoverShareDiff > 0;
      top4SectorDays++;
      if (combinedIsPositive) combinedPositive++;
      if (combinedIsNegative) negativeCombined++;
      if (combinedIsPositive && sharePositive) combinedAndShare++;
      if (bothPositive && sharePositive) bothAndShare++;

      const previous = byIndex.get(day.marketIndex - 1);
      const wasTop = previous?.byKind.FLOW.slice(0, TOP_MAX_RANK).some((item) => item.sectorCode === row.sectorCode) ?? true;
      if (!wasTop) {
        newTop4Entries++;
        if (combinedIsPositive && sharePositive) entryCombinedAndShare++;
        if (bothPositive && sharePositive) entryBothAndShare++;
      }
    }
  }
  ...
  return {
    top4SectorDays,
    combinedNetPositiveRate: rate(combinedPositive, top4SectorDays),
    combinedNetAndSharePositiveRate: rate(combinedAndShare, top4SectorDays),
    bothInvestorAndSharePositiveRate: rate(bothAndShare, top4SectorDays),
    negativeCombinedNetRate: rate(negativeCombined, top4SectorDays),
    newTop4Entries,
    entryCombinedNetAndSharePositiveRate: rate(entryCombinedAndShare, newTop4Entries),
    entryBothInvestorAndSharePositiveRate: rate(entryBothAndShare, newTop4Entries),
  };
}
...
function familyDiagnostic(
  ...
  const strict = strictTopEpisodes(history, sectorDefs, kind);
  const buffered = bufferedTopEpisodes(history, sectorDefs, kind);
  ...
  return {
    ...
    topSetRetention: SET_RETENTION_HORIZONS.map((horizon) => topSetRetention(history, kind, horizon)),
...
export function runSectorRotationDecompositionBacktest(
  ...
  const availableCodes = new Set(
    dataset.instruments.filter((item) => item.instrumentType === "STOCK").map((item) => item.sectorCode),
  );
  const sectorDefs = THEME_SECTORS.filter(
    (sector) =>
      sector.code !== "MARKET_IDX" && sector.code !== "ETC" && availableCodes.has(sector.code),
  );
  if (!sectorDefs.length) return null;

  const prepared = dataset.instruments
    .filter(
      (item) =>
        item.instrumentType === "STOCK" && sectorDefs.some((sector) => sector.code === item.sectorCode),
    )
  ...
  return {
    ...
    families: SCORE_KINDS.map((kind) => familyDiagnostic(history, sectorDefs, kind)),
    flowConfirmation: flowConfirmation(history),
    notes: [
      "Price Leadership, Money Flow, Rotation Score를 각각 독립 순위화해 Top4 지속성을 비교합니다.",
      "Rotation Score는 현재 CloudTrend 기본 가중치(가격 40% + 자금흐름 45% + 로테이션 모멘텀 15%)를 그대로 사용합니다.",
      "4위-5위 점수차가 작을수록 경계 순위 교체가 실제 큰 변화보다 상대순위 노이즈일 가능성이 높습니다.",
      "실질 자금유입 확인은 외국인+기관 5일 합산 순매수 양수와 거래대금 점유율(5일-20일) 상승을 동시에 요구합니다.",
      "더 엄격한 확인치는 외국인과 기관이 모두 5일 순매수이고 거래대금 점유율도 상승한 경우입니다.",
    ],
  };
}
