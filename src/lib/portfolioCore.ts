...
export function createPortfolioStore(deps: {
  supabase: SupabaseClient;
  userId: () => Promise<string>;
  loadSnapshots: () => ScreeningSnapshot[];
  ensureManualDataset: () => Promise<{ dataset: MarketDataset } | null>;
}) {
  const { supabase, userId, loadSnapshots, ensureManualDataset } = deps;
  ...
  function buildSummary(
    ...
    return {
      ...
      totalReturn:
        settings.initialCapital > 0 ? rate((totalPnl / settings.initialCapital) * 100) : 0,
      openPositions,
  ...
  async function savePortfolioCapital(initialCapital: number): Promise<void> {
