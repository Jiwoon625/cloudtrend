...
export async function updatePortfolioEntryExecution(
  ...
  if (!Number.isInteger(shares) || shares <= 0)
    throw new Error("실제 매수수량은 1주 이상의 정수로 입력해 주세요.");
  ...
  const [{ data: settings, error: settingsError }, { data: trade, error: tradeError }] =
    await Promise.all([
      ...
      supabase
        ...
        .select("id,entry_status,status,exit_price")
        .eq("user_id", uid)
        .eq("id", tradeId)
        .single(),
    ]);
  ...
  const baseStatus = String(trade.entry_status ?? "KOSDAQ 8 ONSET").replace(
    /\s*·\s*실제체결 수정$/,
    "",
  );
  ...
  const patch: Record<string, unknown> = {
    ...
    entry_fee: entryFee,
    entry_status: `${baseStatus} · 실제체결 수정`,
