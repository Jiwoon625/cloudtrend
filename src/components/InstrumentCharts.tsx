...
import {
  getCachedInstrumentChart,
  getCachedInstrumentPrices,
  INSTRUMENT_CHART_VERSION,
...
export default function InstrumentCharts({
  ...
  const row = payload.analysis.rows.find((row) => row.instrument.symbol === symbol)!;
  const isEtf = row.instrument.instrumentType === "ETF";
  const scoreMax = isEtf ? 100 : 10;
  const key = [
    "instrument-chart",
    INSTRUMENT_CHART_VERSION,
    symbol,
    payload.analysis.calculatedAt,
    getActiveScoringConfig(),
    range,
  ];
  const priceQuery = useQuery({
    queryKey: [...key, "prices"],
    queryFn: () => getCachedInstrumentPrices(symbol, range, payload),
    staleTime: Infinity,
    gcTime: 30 * 60 * 1000,
    retry: false,
  });
  ...
  const chart = query.data?.chart ?? priceQuery.data?.chart ?? [];
  ...
  return (
    <>
      <section className="mt-5 rounded-lg border border-border bg-card p-4">
        ...
        {query.isPending && priceQuery.isPending ? (
          <div
              ...
              <ComposedChart data={chart}>
                ...
                {visible.technical ? (
                  <YAxis
                    ...
                    orientation="right"
                    domain={[0, scoreMax]}
                    ticks={isEtf ? [0, 20, 40, 60, 80, 100] : [0, 2, 4, 6, 8, 10]}
                    width={48}
                ...
                {visible.technical ? (
                  <Line
                    ...
                    name={isEtf ? "ETF 기술점수 (100점)" : "기술점수 (V8 Final · 10점)"}
                    type="linear"
                    stroke="#f97316"
                    strokeWidth={2.5}
                    strokeDasharray="6 3"
        ...
        {chart.length > 0 && query.isPending ? (
          <p role="status" className="mt-2 text-xs text-muted-foreground">
            가격·거래량 표시 완료 · 기술점수 준비 중…
          </p>
        ) : null}
        {chart.length > 0 && query.isError ? (
          <div role="alert" className="mt-2 text-xs">
            <p>가격·거래량은 표시했지만 기술점수를 불러오지 못했습니다. {query.error.message}</p>
        ...
        <p className="mt-1 text-[11px] text-muted-foreground">
          {isEtf
            ? "오른쪽 축은 카드와 동일한 ETF 기술점수 0~100점입니다."
            : "오른쪽 축은 카드와 동일한 V8 Final 기술점수 0~10점입니다. 52주 신고가, 외국인 20일 순매수와 전체 시장 기준 Sector Price Leadership을 포함합니다."}{" "}
