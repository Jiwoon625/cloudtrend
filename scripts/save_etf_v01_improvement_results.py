...
def main():
    ...
    def archive(prefix,files):
        ...
        return manifest
    root=Path('analysis-runs/m0-v01-improvements');prefix=owner+'/results/etf-m0-v01-improvements/20260911/'+os.environ['GITHUB_RUN_ID']
    archive(owner+'/source/Reference',{'etf-v01-benchmark-duplicate-map-20260911-'+os.environ['GITHUB_RUN_ID']+'.csv':(root/'benchmark-and-duplicate-reference.csv').read_bytes()})
    ...
    (root/'storage-receipt.json').write_text(json.dumps(dict(prefix=prefix,files=len(files),verified=True),indent=2))
