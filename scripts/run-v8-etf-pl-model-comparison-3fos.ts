...
const STUDY_VERSION = "CloudTrend V8 ETF PL Model Comparison 3-FOS" as const;
...
function usage(message?: string): never {
  throw new Error(
    [
      ...(message ? [message, ""] : []),
      "Usage:",
      "  npx vite-node scripts/run-v8-etf-pl-model-comparison-3fos.ts --source-manifest <path> --source-cache-dir <dir> [--supabase-user-id <uuid>] [--upload]",
    ].join("\n"),
  );
}
...
function parseArgs(argv: string[]): Options {
  ...
  if (!options.sourceManifest || !options.sourceCacheDir)
    usage("source manifest and cache dir are required.");
...
async function loadCachedTexts(manifestPath: string, cacheDir: string) {
  ...
  process.stderr.write(
    "ETF PL study source cache verified: " +
      manifest.fileCount +
      " files / " +
...
async function main() {
  ...
  const outputPath = path.join(
    outputDir,
    "v8-etf-pl-model-comparison-3fos-" + runId + ".json",
  );
  ...
  if (options.upload) {
    ...
    remotePath =
      options.userId + "/results/v8-etf-pl-model-comparison-3fos/" + runId + ".json";
    await uploadJson(client, remotePath, result);
    await uploadJson(
      client,
      options.userId + "/results/v8-etf-pl-model-comparison-3fos/latest.json",
      {
        version: STUDY_VERSION,
        createdAt,
        runId,
