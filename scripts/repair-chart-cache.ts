...
if (digest !== saved.resultDigest) {
  const left = deterministicAnalysis(analysis) as any,
    right = deterministicAnalysis(saved.payload.analysis) as any;
  const changed = Object.keys(left).filter(
    (k) => stableCacheJson(left[k]) !== stableCacheJson(right[k]),
  );
  stage("digest-mismatch", { changed });
  for (const k of changed) {
    if (k === "rows") {
      const differences = left.rows.flatMap((row: any) => {
        const other = right.rows.find((r: any) => r.instrument.symbol === row.instrument.symbol);
        if (!other) return [{ symbol: row.instrument.symbol, missing: true }];
        const fields = Object.keys(row).filter(
          (f) => stableCacheJson(row[f]) !== stableCacheJson(other[f]),
        );
        return fields.length ? [{ symbol: row.instrument.symbol, fields }] : [];
      });
...
await warmRecentCharts(client, uid, saved, { analysis, dataset, config });
stage("charts-ready");
