...
const STUDY_VERSION = "CloudTrend V8 ETF PL Four-Candidate All-Year OOS" as const;
...
const ETF_THRESHOLDS = [80, 84, 85, 90] as const;
...
const MODELS: ModelDef[] = [
  {
    id: "A_STOCK_PL",
    label: "A. Existing V8",
    description: "Stock PL only, threshold 80",
    etfThreshold: null,
  },
  ...ETF_THRESHOLDS.map((threshold) => ({
...
async function main() {
  ...
  const result = {
    ...
    design: {
      ...
      thresholdGridFormula:
        "For D models, use ETF PL whenever available and award the 0.5 sector slot only when ETF PL is below the model threshold (80/84/85/90). If ETF PL is missing, fall back to Stock PL with threshold fixed at 80. A baseline uses Stock PL only at threshold 80. OOS years are every complete calendar year with full intended ETF PL warm-up: 2018-2025.",
  ...
  const outputPath = path.join(
    outputDir,
    "v8-etf-pl-four-candidate-all-year-oos-" + runId + ".json",
  );
  ...
  if (options.upload) {
    ...
    remotePath =
      options.userId + "/results/v8-etf-pl-four-candidate-all-year-oos/" + runId + ".json";
    await uploadJson(client, remotePath, result);
    await uploadJson(
      client,
      options.userId + "/results/v8-etf-pl-four-candidate-all-year-oos/latest.json",
      {
        version: STUDY_VERSION,
        createdAt,
        runId,
