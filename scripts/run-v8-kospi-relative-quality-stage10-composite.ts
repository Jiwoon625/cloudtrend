...
function usage(message?: string): never {
  throw new Error(
    [
      ...(message ? [message, ""] : []),
      "Usage:",
      "  npx vite-node --script scripts/run-v8-kospi-relative-quality-stage10-composite.ts --source-manifest <path> --source-cache-dir <dir> [--supabase-user-id <uuid>] [--upload]",
    ].join("\n"),
  );
}
...
function residualMomentum(
  ...
  for (let i = benchmarkIndex - lag + 1; i <= benchmarkIndex; i++) {
    ...
    if (
      ...
      continue;
    residuals.push(
      stockNow.close / stockPrev.close - 1 - beta * (marketNow.close / marketPrev.close - 1),
    );
  }
...
function rawFeatures(
  ...
  if (stockNowIndex === undefined || stock20Index === undefined || stock60Index === undefined)
    return null;
  const stockNow = bars[stockNowIndex];
  ...
  if (
    ...
    !stock60 ||
    !finite(stockNow.close) ||
    !finite(stock20.close) ||
    !finite(stock60.close) ||
    stock20.close <= 0 ||
    stock60.close <= 0 ||
    market20.close <= 0 ||
    market60.close <= 0
    ...
    return null;
  const rs20 =
    (stockNow.close / stock20.close - 1) * 100 - (marketNow.close / market20.close - 1) * 100;
  const rs60 =
    (stockNow.close / stock60.close - 1) * 100 - (marketNow.close / market60.close - 1) * 100;
...
function evaluate(rows: Observation[], feature: Feature) {
  ...
  const sorted = [...valid].sort(
    (a, b) => (valueOf(a, feature) as number) - (valueOf(b, feature) as number),
  );
...
export async function runStudy() {
  ...
  for (const series of kospiSeries) {
    ...
    for (let i = 1; i + 1 < series.bars.length; i++) {
      ...
      if (
        ...
        !(previousScore < ONSET_THRESHOLD && score >= ONSET_THRESHOLD)
      )
        continue;
      rawOnsets++;
  ...
  for (const horizon of HORIZONS)
    addDailyComposite(observations.filter((row) => row.horizon === horizon));
  ...
  const candidateSummary = FEATURES.map((feature) => {
    ...
    return {
      ...
      threeFos20: {
        ...
        positiveSpearmanFolds: folds20.filter(
          (row) => finite(row.overallSpearman) && row.overallSpearman > 0,
        ).length,
        meanDailySpearman: round(
          average(folds20.map((row) => row.meanDailySpearman).filter(finite)),
        ),
        meanTopBottomSpreadPct: round(
          average(folds20.map((row) => row.topBottomAvgSpreadPct).filter(finite)),
        ),
        positiveSpreadFolds: folds20.filter(
          (row) => finite(row.topBottomAvgSpreadPct) && row.topBottomAvgSpreadPct > 0,
        ).length,
        meanCorrelationWithV8Score: round(
          average(folds20.map((row) => row.correlationWithV8Score).filter(finite)),
        ),
      },
      yearly2017to2025: {
  ...
  const result = {
    ...
    design: {
      objective:
        "Test whether beta-adjusted residual acceleration adds information beyond RSAccel and whether a fixed 50:50 daily-rank composite improves KOSPI 8-point onset discrimination.",
      ...
      features: [...FEATURES],
      composite:
        "On each signal date, percentile-rank RSAccel and RESID_ACCEL among strict V8 8-point KOSPI onset candidates and average the two ranks 50:50. No weight search is performed.",
      partialCorrelation:
        "Spearman partial-correlation diagnostic using pairwise rank correlations; used only to test incremental information, not as a production estimator.",
      target:
        "Future stock return minus KOSPI return over identical NEXT_OPEN to horizon-close dates.",
      pointInTime:
        "All inputs use information available on or before signal close; execution is NEXT_OPEN.",
    },
    candidateSummary,
    diagnosticSummary,
    holdout2026,
  ...
  const outputPath = path.join(
    outputDir,
    `v8-kospi-relative-quality-stage10-composite-${runId}.json`,
  );
  ...
  if (options.upload) {
    ...
    await uploadJson(
      client,
      `${options.userId}/results/v8-kospi-relative-quality-stage10-composite/latest.json`,
      result,
    );
  }
  process.stdout.write(
    `${JSON.stringify({ outputPath, remotePath, candidateSummary, diagnosticSummary }, null, 2)}\n`,
  );
}

if (process.argv[1]?.endsWith("run-v8-kospi-relative-quality-stage10-composite.ts"))
