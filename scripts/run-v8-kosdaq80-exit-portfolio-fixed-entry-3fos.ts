...
const EXIT_SCENARIOS: ExitScenario[] = [
  { id: "UP95_H60", label: "KOSDAQ80 · ↑95 / max 60D", upsideExitThreshold: 9.5, downsideExitThreshold: null, maxHoldingDays: 60 },
  { id: "DOWN25_H40", label: "KOSDAQ80 · ↓25 / max 40D", upsideExitThreshold: null, downsideExitThreshold: 2.5, maxHoldingDays: 40 },
  { id: "TIME60", label: "KOSDAQ80 · TIME ONLY / 60D", upsideExitThreshold: null, downsideExitThreshold: null, maxHoldingDays: 60 },
  { id: "BOTH95_DOWN25_H60", label: "KOSDAQ80 · ↑95 or ↓25 / max 60D", upsideExitThreshold: 9.5, downsideExitThreshold: 2.5, maxHoldingDays: 60 },
];
...
function median(values: number[]) { if (!values.length) return null; const sorted = [...values].sort((a, b) => a - b); const mid = (sorted.length - 1) / 2; return (sorted[Math.floor(mid)]! + sorted[Math.ceil(mid)]!) / 2; }
function round(value: number | null, digits = 6) { if (!finite(value)) return null; const factor = 10 ** digits; return Math.round(value * factor) / factor; }
...
async function loadCachedTexts(manifestPath: string, cacheDir: string) {
  ...
  process.stderr.write(`V8-11b source cache verified: ${manifest.fileCount} files / ${(manifest.totalBytes / 1_000_000).toFixed(1)} MB\n`);
...
function buildCandidate(signal: EntrySignal, scenario: ExitScenario): PortfolioCandidateTrade | null {
  ...
  for (let j = entryIndex; j <= Math.min(plannedExit, series.bars.length - 1); j++) {
    ...
    if (![bar.open, bar.close, bar.low, bar.high].every((value) => finite(value) && value > 0)) return null;
    if (j > entryIndex) {
      const scoreIndex = j - 1;
      const previous = adjustedScoreAt(series, scoreIndex - 1);
      const current = adjustedScoreAt(series, scoreIndex);
      if (scenario.downsideExitThreshold !== null && crossedDown(previous, current, scenario.downsideExitThreshold)) {
        exitIndex = j; exitPrice = bar.open; exitReason = "DOWNSIDE_SCORE"; exitTiming = "OPEN"; break;
      }
      if (scenario.upsideExitThreshold !== null && crossedUp(previous, current, scenario.upsideExitThreshold)) {
...
function lookupSeries(series: PortfolioSeries[]) { return new Map(series.map((item) => [key(item.market, item.symbol), item])); }
...
function candidatePriority(a: PortfolioCandidateTrade, b: PortfolioCandidateTrade) {
  ...
  const aRise = a.scoreRise5d ?? -Infinity, bRise = b.scoreRise5d ?? -Infinity;
...
function peakCandidateConcurrency(candidates: PortfolioCandidateTrade[], dates: string[]) {
  ...
  for (const candidate of candidates) { const list = byEntry.get(candidate.entryDate) ?? []; list.push(candidate); byEntry.set(candidate.entryDate, list); }
  for (const list of byEntry.values()) list.sort(candidatePriority);
  const active = new Map<string, PortfolioCandidateTrade>();
  let peak = 0;
  for (const date of dates) {
    for (const [positionKey, candidate] of [...active]) if (candidate.exitDate === date && candidate.exitTiming === "OPEN") active.delete(positionKey);
    for (const candidate of byEntry.get(date) ?? []) { const positionKey = key(candidate.market, candidate.symbol); if (!active.has(positionKey)) active.set(positionKey, candidate); }
    peak = Math.max(peak, active.size);
    for (const [positionKey, candidate] of [...active]) if (candidate.exitDate === date && candidate.exitTiming === "CLOSE") active.delete(positionKey);
  }
  return Math.max(1, peak);
}

function simulatePortfolio(candidates: PortfolioCandidateTrade[], series: PortfolioSeries[], dates: string[], capacity: CapacitySpec, roundTripCostBps: number): SimulationResult {
  ...
  let cash = INITIAL_CAPITAL, previousEquity = INITIAL_CAPITAL, peakEquity = INITIAL_CAPITAL, totalFees = 0;
  let skippedForCapacity = 0, skippedAlreadyHeld = 0, skippedForCash = 0;
  ...
  for (const date of dates) {
    ...
    for (const candidate of byEntry.get(date) ?? []) {
      ...
      if (positions.has(positionKey)) { skippedAlreadyHeld += 1; continue; }
      if (capacity.maxPositions !== null && positions.size >= capacity.maxPositions) { skippedForCapacity += 1; continue; }
      ...
      if (!(entryNotional > 1)) { skippedForCash += 1; continue; }
      const shares = entryNotional / candidate.entryPrice;
      const entryFee = entryNotional * halfCost;
      cash -= entryNotional + entryFee;
      totalFees += entryFee;
      positions.set(positionKey, { candidate, shares, entryNotional, entryFee, lastMark: candidate.entryPrice });
    }
    ...
    points.push({ date, equity, cashWeight: equity > 0 ? cash / equity * 100 : 100, activePositions: positions.size, drawdown });
    previousEquity = equity;
    void previousEquity;
  }
  return { points, trades, candidateSignals: candidates.length, skippedForCapacity, skippedAlreadyHeld, skippedForCash, totalFees, peakCandidateConcurrency: peakConcurrency, slotCount };
}
...
function benchmarkReturn(dataset: MarketDataset, startDate: string, endDate: string) {
  ...
  const start = map.get(startDate), end = map.get(endDate);
...
function makeFoldRow(scenario: ExitScenario, fold: FoldYear, capacity: CapacitySpec, simulation: SimulationResult, zeroCost: SimulationResult, dataset: MarketDataset): FoldRow {
  ...
  const startDate = points[0]?.date, endDate = points.at(-1)?.date;
  ...
  return {
    scenario: scenario.id, scenarioLabel: scenario.label, fold, capacity: capacity.id, capacityLabel: capacity.label,
    maxPositions: capacity.maxPositions, slotCount: simulation.slotCount, slotWeightPct: round(100 / simulation.slotCount) ?? 0,
    peakCandidateConcurrency: simulation.peakCandidateConcurrency, candidateSignals: simulation.candidateSignals,
    trades: simulation.trades.length, skippedForCapacity: simulation.skippedForCapacity, skippedAlreadyHeld: simulation.skippedAlreadyHeld,
    skippedForCash: simulation.skippedForCash, totalReturn: round(totalReturn), zeroCostTotalReturn: round(zeroCostTotalReturn),
    costDragPctPoint: finite(totalReturn) && finite(zeroCostTotalReturn) ? round(totalReturn - zeroCostTotalReturn) : null,
    benchmarkReturn: round(benchmark), portfolioExcessReturn: finite(totalReturn) && finite(benchmark) ? round(totalReturn - benchmark) : null,
    cagr: round(cagr), mdd: round(mdd), medianTradeReturn: round(median(netReturns)), avgTradeReturn: round(average(netReturns)),
    winRate: netReturns.length ? round((netReturns.filter((value) => value > 0).length / netReturns.length) * 100) : null,
    profitFactor: round(profitFactor(netReturns)), averageHoldingDays: round(average(simulation.trades.map((trade) => trade.holdingDays))),
    ...
    activeDayRate: points.length ? round((activeDays / points.length) * 100) : null, totalFees: round(simulation.totalFees, 2) ?? 0,
  };
}

function aggregateRows(rows: FoldRow[]): AggregateRow[] {
  const out: AggregateRow[] = [];
  for (const scenario of EXIT_SCENARIOS) for (const capacity of CAPACITY_SPECS) {
    ...
    out.push({
      scenario: scenario.id, scenarioLabel: scenario.label, capacity: capacity.id, capacityLabel: capacity.label, folds: selected.length,
      ...
      avgTotalReturn: round(average(totalReturns)), medianFoldTotalReturn: round(median(totalReturns)),
      avgPortfolioExcessReturn: round(average(excessReturns)), medianFoldExcessReturn: round(median(excessReturns)),
      avgCagr: round(average(selected.map((row) => row.cagr).filter(finite))), avgMdd: round(average(selected.map((row) => row.mdd).filter(finite))),
      ...
      avgTrades: round(average(selected.map((row) => row.trades))), totalCapacitySkips: selected.reduce((sum, row) => sum + row.skippedForCapacity, 0),
...
async function main() {
  ...
  const foldSignalCounts = FOLD_YEARS.map((fold) => ({ fold, signals: fixedSignals.filter((signal) => Number(signal.signalDate.slice(0, 4)) === fold).length }));
  const foldRows: FoldRow[] = [];
  const candidateCounts: Array<{ scenario: ExitScenarioId; fold: FoldYear; signals: number; candidates: number; peakConcurrency: number }> = [];

  for (const scenario of EXIT_SCENARIOS) {
    for (const fold of FOLD_YEARS) {
      const foldSignals = fixedSignals.filter((signal) => Number(signal.signalDate.slice(0, 4)) === fold);
      const candidates = foldSignals.map((signal) => buildCandidate(signal, scenario)).filter((candidate): candidate is PortfolioCandidateTrade => candidate !== null);
      const dates = datesForCandidates(context.allDates, candidates);
      const peakConcurrency = peakCandidateConcurrency(candidates, dates);
      candidateCounts.push({ scenario: scenario.id, fold, signals: foldSignals.length, candidates: candidates.length, peakConcurrency });
  ...
  const result = {
    version: STUDY_VERSION, createdAt, runId,
    data: { datasetVersion: dataset.version, asOfDate: dataset.asOfDate, sourceCacheKey: manifest.cacheKey, sourceFiles: manifest.fileCount, sourceBytes: manifest.totalBytes, symbolCount: context.symbolCount },
    design: {
      folds: [...FOLD_YEARS], market: "KOSDAQ", onsetThresholdPercent: 80,
      entryUniverse: "One fixed KOSDAQ80 onset signal set shared by every exit scenario. No entry is excluded because its signal-day score already exceeds an exit threshold.",
      scoreModel: "Frozen V8 10-point score. Sector PL missing contributes 0; PL<80 contributes +0.5; PL>=80 contributes +0.",
      entryExecution: "NEXT_OPEN", scoreExitExecution: "NEXT_OPEN after threshold crossing on prior close", timeExitExecution: "SAME_DAY_CLOSE at max holding",
      roundTripCostBps: ROUND_TRIP_COST_BPS, initialCapital: INITIAL_CAPITAL,
      positionSizing: "Fixed slot weights: 10=10%, 20=5%, 30=3.333%. UNCAPPED uses 1 / ex-post peak candidate concurrency within each fold/scenario as a diagnostic benchmark only.",
      fractionalShares: true, duplicateEntryAllowed: false,
      priority: ["adjustedScore10 desc", "scoreRise5d desc", "signalTradingValue desc", "symbol asc"],
    },
    foldSignalCounts, scenarios: EXIT_SCENARIOS, capacities: CAPACITY_SPECS, candidateCounts, aggregateRows: aggregate, foldRows,
    notes: [
  ...
  const outputPath = path.join(outputDir, `v8-11b-kosdaq80-exit-portfolio-fixed-entry-3fos-${runId}.json`);
  ...
  if (options.upload) {
    ...
    await uploadJson(client, `${options.userId}/results/v8-11b-kosdaq80-exit-portfolio-fixed-entry-3fos/latest.json`, { version: STUDY_VERSION, createdAt, runId, resultPath: remotePath, foldSignalCounts, aggregateRows: aggregate });
  }
  process.stdout.write(`${JSON.stringify({ outputPath, remotePath, foldSignalCounts, candidateCounts, aggregateRows: aggregate }, null, 2)}\n`);
}
