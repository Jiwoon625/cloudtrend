...
const STUDY_VERSION = "CloudTrend V8 ETF PL Threshold 80-90 Grid 3-FOS" as const;
...
const ETF_THRESHOLDS = [80, 81, 82, 83, 84, 85, 86, 87, 88, 89, 90] as const;
...
async function main() {
  ...
  const result = {
    ...
    design: {
      ...
      thresholdGridFormula:
        "For D models, use ETF PL whenever available and award the 0.5 sector slot only when ETF PL is below the model threshold (80/81/82/83/84/85/86/87/88/89/90). If ETF PL is missing, fall back to Stock PL with threshold fixed at 80. A baseline uses Stock PL only at threshold 80.",
  ...
  const outputPath = path.join(
    outputDir,
    "v8-etf-pl-threshold-80-90-grid-3fos-" + runId + ".json",
  );
  ...
  if (options.upload) {
    ...
    remotePath =
      options.userId + "/results/v8-etf-pl-threshold-80-90-grid-3fos/" + runId + ".json";
    await uploadJson(client, remotePath, result);
    await uploadJson(
      client,
      options.userId + "/results/v8-etf-pl-threshold-80-90-grid-3fos/latest.json",
      {
        version: STUDY_VERSION,
        createdAt,
        runId,
