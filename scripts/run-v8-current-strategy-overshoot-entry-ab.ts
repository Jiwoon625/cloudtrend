...
const STUDY_VERSION = "CloudTrend V8 Current Strategy Overshoot Entry A/B" as const;
...
type OvershootPolicy = "ALLOW" | "SKIP_OVERSHOOT";
...
async function main() {
  ...
  const policies: OvershootPolicy[] = ["ALLOW", "SKIP_OVERSHOOT"];
  ...
  for (const strategy of strategies) {
    for (const fold of FOLD_YEARS) {
      const allowCandidates = buildCandidates(
        modelSeries,
        strategy.market,
        fold,
        stockPl,
      ...
      const overshootCandidates = allowCandidates.filter((candidate) => candidate.overshoot);
      const overshootReturns = overshootCandidates.map((candidate) => candidate.grossReturn);
      const nonOvershootReturns = allowCandidates
        .filter((candidate) => !candidate.overshoot)
        .map((candidate) => candidate.grossReturn);

      signalDiagnostics.push({
        strategy: strategy.id,
        market: strategy.market,
        fold,
        totalOnsetSignals: allowCandidates.length,
        overshootSignals: overshootCandidates.length,
        overshootRate: allowCandidates.length
          ? round((overshootCandidates.length / allowCandidates.length) * 100)
          : null,
        overshootAvgGrossReturn: round(average(overshootReturns)),
        overshootMedianGrossReturn: round(median(overshootReturns)),
        overshootWinRate: overshootReturns.length
          ? round((overshootReturns.filter((value) => value > 0).length / overshootReturns.length) * 100)
          : null,
        nonOvershootAvgGrossReturn: round(average(nonOvershootReturns)),
        nonOvershootMedianGrossReturn: round(median(nonOvershootReturns)),
        nonOvershootWinRate: nonOvershootReturns.length
          ? round((nonOvershootReturns.filter((value) => value > 0).length / nonOvershootReturns.length) * 100)
          : null,
      });
      ...
      for (const policy of policies) {
        const candidates =
          policy === "ALLOW"
            ? allowCandidates
            : allowCandidates.filter((candidate) => !candidate.overshoot);
        ...
        if (lastExit && dates.at(-1) && lastExit > dates.at(-1)!) {
          throw new Error(
            `Evaluation calendar too short for ${strategy.id} ${policy} ${fold}: ${lastExit} > ${dates.at(-1)}`,
          );
        }
  ...
  function summarizeRows(selected: typeof foldRows) {
    const avg = (pick: (row: (typeof selected)[number]) => number | null) =>
      round(average(selected.map(pick).filter(finite)));
    const totalOnsets = selected.reduce((sum, row) => sum + row.totalOnsetSignals, 0);
    const totalOvershoots = selected.reduce((sum, row) => sum + row.overshootSignals, 0);
    return {
      folds: selected.length,
      years: selected.map((row) => row.fold),
      ...
      avgHoldingDays: avg((row) => row.avgHoldingDays),
      avgCapitalOccupancy: avg((row) => row.avgCapitalOccupancy),
      foldsPositiveReturn: selected.filter((row) => finite(row.totalReturn) && row.totalReturn > 0)
        .length,
      ...
      totalOnsetSignals: totalOnsets,
      totalOvershootSignals: totalOvershoots,
      overshootRate: totalOnsets ? round((totalOvershoots / totalOnsets) * 100) : null,
    };
  }
  ...
  const aggregateRows = strategies.flatMap((strategy) =>
    policies.map((policy) => {
      const all = foldRows.filter(
        (row) => row.strategy === strategy.id && row.overshootPolicy === policy,
      );
      return {
        strategy: strategy.id,
        market: strategy.market,
  ...
  const comparisonRows = strategies.map((strategy) => {
    const allow = aggregateRows.find(
      (row) => row.strategy === strategy.id && row.overshootPolicy === "ALLOW",
    )!;
    const skip = aggregateRows.find(
      (row) => row.strategy === strategy.id && row.overshootPolicy === "SKIP_OVERSHOOT",
    )!;
    const deltaPanel = (
      ...
      const d = (x: number | null, y: number | null) =>
        finite(x) && finite(y) ? round(x - y) : null;
      return {
        totalReturnDeltaPp: d(b.avgTotalReturn, a.avgTotalReturn),
        excessReturnDeltaPp: d(b.avgExcessReturn, a.avgExcessReturn),
        cagrDeltaPp: d(b.avgCagr, a.avgCagr),
        sharpeDelta: d(b.avgSharpe, a.avgSharpe),
        mddDeltaPp: d(b.avgMdd, a.avgMdd),
        worstMddDeltaPp: d(b.worstFoldMdd, a.worstFoldMdd),
        tradeReturnDeltaPp: d(b.avgTradeReturn, a.avgTradeReturn),
        winRateDeltaPp: d(b.avgWinRate, a.avgWinRate),
        profitFactorDelta: d(b.avgProfitFactor, a.avgProfitFactor),
    ...
    return {
      strategy: strategy.id,
      market: strategy.market,
      comparison: "SKIP_OVERSHOOT_MINUS_ALLOW",
      with2025: deltaPanel(allow.with2025, skip.with2025),
      without2025: deltaPanel(allow.without2025, skip.without2025),
    };
  });
  ...
  const result = {
    ...
    design: {
      ...
      portfolio: "P30 equal-slot diagnostic portfolio: max 30 positions, fractional shares, no rebalancing or sector cap; candidate-level diagnostics are reported separately.",
      evaluationCalendar:
    ...
    foldRows,
    notes: [
      "This is a one-variable A/B test: ALLOW vs SKIP_OVERSHOOT. All score, exit, fill, cost and holding rules are identical.",
  ...
  const outputPath = path.join(
    outputDir,
    "v8-current-strategy-overshoot-entry-ab-" + runId + ".json",
  );
  ...
  if (options.upload) {
    ...
    remotePath =
      options.userId + "/results/v8-current-strategy-overshoot-entry-ab/" + runId + ".json";
    await uploadJson(client, remotePath, result);
    await uploadJson(
      client,
      options.userId + "/results/v8-current-strategy-overshoot-entry-ab/latest.json",
      {
        version: STUDY_VERSION,
        createdAt,
        runId,
