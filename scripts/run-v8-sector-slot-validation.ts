...
function usage(): never {
  throw new Error([
    "Usage:",
    "  npx vite-node scripts/run-v8-sector-slot-validation.ts --supabase-user-id <uuid> [--upload]",
    "Options:",
    "  --output <dir>  default: v8-sector-slot-runs",
...
function parseArgs(argv: string[]): Options {
  const options: Options = {
    supabaseUserId: null,
    outputRoot: "v8-sector-slot-runs",
    upload: false,
    limit: 613,
    roundTripCostBps: 0,
  };
...
async function main() {
  ...
  if (!result) throw new Error("V8 Sector Price Leadership 0.5점 검증 결과를 계산하지 못했습니다.");
  ...
  await writeFile(path.join(outputDir, "sector-slot-validation.json"), JSON.stringify(payload, null, 2));

  let remotePath: string | null = null;
  if (options.upload) {
    remotePath = `${options.supabaseUserId}/results/v8-sector-slot-validation/latest.json`;
    await uploadJson(client, remotePath, payload);
  }

  const keyForward = result.forwardRows.filter((row) =>
    row.scope === "SPLIT" && row.split === "OOS" &&
    (row.market === "KOSPI" || row.market === "KOSDAQ") &&
    [20, 40].includes(row.horizon),
  );
  const keyStrategy = result.strategyRows.filter((row) => row.scope === "SPLIT" && row.split === "OOS");
  const yearlyStrategy = result.strategyRows.filter((row) => row.scope === "YEAR");
  const robustness = result.policies.flatMap((policy) =>
    (["KOSPI", "KOSDAQ"] as const).flatMap((market) => {
      const entries = market === "KOSPI" ? [65, 75] : [75, 80];
      return entries.map((entryThreshold) => {
        const rows = yearlyStrategy.filter((row) => row.policyId === policy.id && row.market === market && row.entryThreshold === entryThreshold && row.acceptedTrades > 0);
        return {
          policyId: policy.id,
          market,
          entryThreshold,
...
// Workflow trigger marker for V8-5 sector slot validation.
