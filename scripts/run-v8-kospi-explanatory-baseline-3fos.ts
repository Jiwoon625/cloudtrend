...
function usage(message?: string): never {
  throw new Error(
    [
      ...(message ? [message, ""] : []),
      "Usage:",
      "  npx vite-node --script scripts/run-v8-kospi-explanatory-baseline-3fos.ts --source-manifest <path> --source-cache-dir <dir> [--supabase-user-id <uuid>] [--upload]",
    ].join("\n"),
  );
}
...
function monotonicity(
  bucketMetrics: ReturnType<typeof bucketRows>,
  field: "avgReturn" | "avgExcessReturn",
) {
  const valid = bucketMetrics
    .map((row, index) => ({ x: index, y: row[field] }))
    .filter((row): row is { x: number; y: number } => finite(row.y));
...
function foldRows(rows: Observation[]) {
  return FOLD_YEARS.flatMap((fold) =>
    HORIZONS.map((horizon) => {
      const selected = rows.filter(
        (row) =>
          Number(row.date.slice(0, 4)) === fold &&
          (row as Observation & { horizon?: Horizon }).horizon === undefined,
      );
...
export async function runStudy() {
  ...
  const aggregates = HORIZONS.map((horizon) => {
    ...
    const bucketAggregate = SCORE_BUCKETS.map((bucket) => {
      const rows = selected
        .map((fold) => fold.scoreBuckets.find((row) => row.bucket === bucket.id)!)
    ...
    return {
      ...
      meanFoldMonotonicityAbsolute: round(
        average(selected.map((row) => row.monotonicity.absoluteReturn).filter(finite)),
      ),
      meanFoldMonotonicityExcess: round(
        average(selected.map((row) => row.monotonicity.excessReturn).filter(finite)),
      ),
      meanFoldDailyIcAbsolute: round(
        average(selected.map((row) => row.dailySpearmanIc.absoluteReturn.mean).filter(finite)),
      ),
      meanFoldDailyIcExcess: round(
        average(selected.map((row) => row.dailySpearmanIc.excessReturn.mean).filter(finite)),
      ),
      onset8: {
        ...
        equalWeightAvgExcessReturn: round(
          average(onsetRows.map((row) => row.avgExcessReturn).filter(finite)),
        ),
        equalWeightExcessWinRate: round(
          average(onsetRows.map((row) => row.excessWinRate).filter(finite)),
        ),
      },
    };
  });
  ...
  const result = {
    ...
    design: {
      ...
      horizons: [...HORIZONS],
      score:
        "Current strict V8 10-point score = historical base 9.5 + 0.5 sector slot when sector PL is available and <80; PL>=80 receives no sector slot.",
      ...
      modelSelectionRule:
        "Baseline measurement only. No score weights or thresholds are changed in this study.",
    },
    aggregates,
    folds,
    notes: [
  ...
  if (options.upload) {
    ...
    await uploadJson(
      client,
      `${options.userId}/results/v8-kospi-explanatory-baseline-3fos/latest.json`,
      {
        version: STUDY_VERSION,
        createdAt,
        runId,
...
if (process.argv[1]?.endsWith("run-v8-kospi-explanatory-baseline-3fos.ts"))
