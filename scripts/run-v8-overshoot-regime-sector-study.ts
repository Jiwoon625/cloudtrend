...
const STUDY_VERSION = "CloudTrend V8 Overshoot Regime Sector Concentration Study" as const;
...
type OvershootPolicyId = "NONE" | "RISK_ON_ONLY" | "NEUTRAL_ONLY" | "RISK_OFF_ONLY" | "RISK_ON_NEUTRAL" | "RISK_ON_RISK_OFF" | "NEUTRAL_RISK_OFF" | "ALL";
...
const OVERSHOOT_POLICIES: Array<{
  id: OvershootPolicyId;
  allowed: MarketRegime[];
}> = [
  { id: "NONE", allowed: [] },
  { id: "RISK_ON_ONLY", allowed: ["RISK_ON"] },
  { id: "NEUTRAL_ONLY", allowed: ["NEUTRAL"] },
  { id: "RISK_OFF_ONLY", allowed: ["RISK_OFF"] },
  { id: "RISK_ON_NEUTRAL", allowed: ["RISK_ON", "NEUTRAL"] },
  { id: "RISK_ON_RISK_OFF", allowed: ["RISK_ON", "RISK_OFF"] },
  { id: "NEUTRAL_RISK_OFF", allowed: ["NEUTRAL", "RISK_OFF"] },
  { id: "ALL", allowed: ["RISK_ON", "NEUTRAL", "RISK_OFF"] },
];

const SECTOR_CAPS = [0.07, 0.10, 0.20, 0.30, 0.40, 1.00] as const;

function filterOvershootPolicy(candidates: Candidate[], allowed: MarketRegime[]) {
  const set = new Set(allowed);
  return candidates.filter((candidate) => !candidate.overshoot || set.has(candidate.regime));
}
...
async function main() {
  ...
  const foldRows: Array<
    FoldRow & {
      strategy: string;
      overshootPolicy: OvershootPolicyId;
      sectorCap: number;
      totalOnsetSignals: number;
      overshootSignals: number;
      acceptedOvershootSignals: number;
    }
  > = [];
  ...
  for (const strategy of strategies) {
    for (const fold of FOLD_YEARS) {
      ...
      for (const regime of ["RISK_ON", "NEUTRAL", "RISK_OFF"] as const) {
        const region = allCandidates.filter((candidate) => candidate.regime === regime);
        const overs = region.filter((candidate) => candidate.overshoot);
        const normal = region.filter((candidate) => !candidate.overshoot);
        const summarizeCandidateSet = (items: Candidate[]) => {
          const returns = items.map((candidate) => candidate.grossReturn);
          return {
            n: items.length,
            avgGrossReturn: round(average(returns)),
            medianGrossReturn: round(median(returns)),
            winRate: returns.length
              ? round((returns.filter((value) => value > 0).length / returns.length) * 100)
              : null,
            avgHoldingDays: round(average(items.map((candidate) => candidate.holdingDays))),
          };
        };
        signalDiagnostics.push({
          ...
          fold,
          regime,
          all: summarizeCandidateSet(region),
          overshoot: summarizeCandidateSet(overs),
          nonOvershoot: summarizeCandidateSet(normal),
          overshootRate: region.length ? round((overs.length / region.length) * 100) : null,
        });
      }

      const grouped = new Map<string, Candidate[]>();
      for (const candidate of allCandidates) {
        const key = candidate.signalDate + "|" + candidate.sectorCode;
      ...
      for (const group of grouped.values()) {
        ...
        const sorted = [...group].sort(candidatePriority);
        const nonOvers = sorted.filter((candidate) => !candidate.overshoot);
        const basket = (items: Candidate[]) =>
          items.length ? round(average(items.map((candidate) => candidate.grossReturn))) : null;
        eventRows.push({
          strategy: strategy.id,
          market: strategy.market,
          fold,
          signalDate: sorted[0]!.signalDate,
          sectorCode: sorted[0]!.sectorCode,
          regime: sorted[0]!.regime,
          size: sorted.length,
          overshootCount: sorted.filter((candidate) => candidate.overshoot).length,
          all: basket(sorted),
          top2: basket(sorted.slice(0, 2)),
          top1: basket(sorted.slice(0, 1)),
          skipOvershoot: basket(nonOvers),
        });
      }

      const dates = evaluationDates(dataset, strategy.market, fold);
      for (const policy of OVERSHOOT_POLICIES) {
        const filtered = filterOvershootPolicy(allCandidates, policy.allowed);
        for (const sectorCap of SECTOR_CAPS) {
          const lastExit = filtered.reduce(
            (max, item) => (item.exitDate > max ? item.exitDate : max),
            "",
          );
          if (lastExit && dates.at(-1) && lastExit > dates.at(-1)!) {
            throw new Error(
              `Evaluation calendar too short for ${strategy.id} ${policy.id} cap=${sectorCap} ${fold}: ${lastExit} > ${dates.at(-1)}`,
            );
          }
          const simulation = simulate(modelSeries, filtered, dates, sectorCap);
          foldRows.push({
            ...foldRow(
            ...
            strategy: strategy.id,
            overshootPolicy: policy.id,
            sectorCap,
            totalOnsetSignals: allCandidates.length,
            overshootSignals: allCandidates.filter((candidate) => candidate.overshoot).length,
            acceptedOvershootSignals: filtered.filter((candidate) => candidate.overshoot).length,
          });
        }
      }
    }
  ...
  function summarizeRows(selected: typeof foldRows) {
    ...
    return {
      ...
      foldsPositiveExcess: selected.filter((row) => finite(row.excessReturn) && row.excessReturn > 0)
        .length,
      ...
      acceptedOvershootSignals: selected.reduce((sum, row) => sum + row.acceptedOvershootSignals, 0),
    };
  }

  const aggregateRows = strategies.flatMap((strategy) =>
    OVERSHOOT_POLICIES.flatMap((policy) =>
      SECTOR_CAPS.map((sectorCap) => {
        const all = foldRows.filter(
          (row) =>
            row.strategy === strategy.id &&
            row.overshootPolicy === policy.id &&
            row.sectorCap === sectorCap,
        );
        return {
          strategy: strategy.id,
          market: strategy.market,
          overshootPolicy: policy.id,
          allowedRegimes: policy.allowed,
          sectorCap,
          with2025: summarizeRows(all),
          without2025: summarizeRows(all.filter((row) => row.fold !== 2025)),
        };
      }),
    ),
  );

  function summarizeEvents(rows: typeof eventRows) {
    const policyKeys = ["all", "top2", "top1", "skipOvershoot"] as const;
    const result: Record<string, unknown> = { events: rows.length };
    for (const key of policyKeys) {
      const values = rows.map((row) => row[key]).filter(finite);
      result[key] = {
        n: values.length,
        avgBasketReturn: round(average(values)),
        medianBasketReturn: round(median(values)),
        positiveEventRate: values.length
  ...
  const sameSectorEventSummary = strategies.flatMap((strategy) =>
    (["RISK_ON", "NEUTRAL", "RISK_OFF"] as const).map((regime) => {
      const rows = eventRows.filter(
        (row) => row.strategy === strategy.id && row.regime === regime,
      );
      const withOvershoot = rows.filter((row) => row.overshootCount > 0);
      const todayAnalog = rows.filter((row) => row.size === 3 && row.overshootCount === 1);
      return {
        strategy: strategy.id,
        market: strategy.market,
        regime,
        allSameSector3Plus: summarizeEvents(rows),
        sameSector3PlusWithOvershoot: summarizeEvents(withOvershoot),
        todayAnalogExactly3OneOvershoot: summarizeEvents(todayAnalog),
      };
    }),
  );

  const regimeCoverage = (["RISK_ON", "NEUTRAL", "RISK_OFF"] as const).map((regime) => ({
    regime,
    dates: [...regimeByDate.values()].filter((row) => row.status === regime).length,
  }));
  ...
  const result = {
    ...
    design: {
      strategies,
      overshootPolicies: OVERSHOOT_POLICIES,
      ...
      portfolio: "P30 equal-slot diagnostic portfolio. Sector caps 7/10/20/30/40/100% are tested; existing positions and same-day entrants count toward the cap.",
      evaluationCalendar:
    ...
    regimeCoverage,
    signalDiagnostics,
    sameSectorEventSummary,
    aggregateRows,
    foldRows,
    notes: [
      "Overshoot means the E8 signal-day score is already at or above the market upside-exit threshold: KOSPI >=9.5, KOSDAQ >=9.0.",
      "Historical market regime replicates the production four-factor gate: KOSPI above MA60, KOSPI above cloud, volatility <30, foreign 5D net buy >0. Backtest data have no VKOSPI, so the engine fallback realized-volatility proxy is used.",
      "All eight subsets of RISK_ON/NEUTRAL/RISK_OFF in which overshoot entry is allowed are tested.",
      "Same-day same-sector groups of 3+ onset signals are separately evaluated as ALL / top2 / top1 / skip-overshoot baskets under the current exit rules.",
      "KOSPI uses ETF PL 84 primary with Stock PL 80 fallback; KOSDAQ uses Stock PL 80 only.",
      ...
      "Both policies use an identical market/fold evaluation calendar and P30 equal-slot diagnostic portfolio implementation.",
  ...
  const outputPath = path.join(
    outputDir,
    "v8-overshoot-regime-sector-study-" + runId + ".json",
  );
  ...
  if (options.upload) {
    ...
    remotePath =
      options.userId + "/results/v8-overshoot-regime-sector-study/" + runId + ".json";
    await uploadJson(client, remotePath, result);
    await uploadJson(
      client,
      options.userId + "/results/v8-overshoot-regime-sector-study/latest.json",
      {
        version: STUDY_VERSION,
        createdAt,
        runId,
