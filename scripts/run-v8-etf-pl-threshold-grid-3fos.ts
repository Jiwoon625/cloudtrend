...
const STUDY_VERSION = "CloudTrend V8 ETF PL Threshold Grid 3-FOS" as const;
...
const ETF_THRESHOLDS = [70, 75, 80, 85, 90] as const;
...
async function main() {
  ...
  const result = {
    ...
    design: {
      ...
      thresholdGridFormula:
        "For D models, use ETF PL whenever available and award the 0.5 sector slot only when ETF PL is below the model threshold (70/75/80/85/90). If ETF PL is missing, fall back to Stock PL with threshold fixed at 80. A baseline uses Stock PL only at threshold 80.",
    ...
    notes: [
      ...
      "D grid models differ only in the ETF PL overheat threshold: 70, 75, 80, 85, or 90.",
  ...
  const outputPath = path.join(
    outputDir,
    "v8-etf-pl-threshold-grid-3fos-" + runId + ".json",
  );
  ...
  if (options.upload) {
    ...
    remotePath =
      options.userId + "/results/v8-etf-pl-threshold-grid-3fos/" + runId + ".json";
    await uploadJson(client, remotePath, result);
    await uploadJson(
      client,
      options.userId + "/results/v8-etf-pl-threshold-grid-3fos/latest.json",
      {
        version: STUDY_VERSION,
        createdAt,
        runId,
